...
class AlbumWidgetSongs(Gtk.Grid):
	...
	def _add_tracks(self, album_id):
		for (id, name, filepath, length, year) in self._db.get_tracks_by_album(album_id):
			ui = Gtk.Builder()
			...
			if not id == self._player.current_track:
			...
			track_widget.checkButton.set_visible(False)
